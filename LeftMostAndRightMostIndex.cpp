...
class Solution
{
    ...
    long long firstOccurence(vector<long long> arr, int n, int x){
    ...
    long long lastOccurence(vector<long long> arr, int n, int x){
        long long low=0, high=n-1, ans=-1;
        while(low<=high){
           int mid=low+(high-low)/2;
            if(arr[mid]==x){
    ...
    {
        int n=v.size();
        return {firstOccurence(v,n,x), lastOccurence(v,n,x)};
    }
};
