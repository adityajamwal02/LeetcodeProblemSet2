...
class Solution{
  ...
  long long int wineSelling(vector<int>& Arr, int N){
      ...
      long long int result=0, k=0;
      for(int i=0;i<pos.size();i++){
          for(int j=k;j<neg.size();j++){
              if(pos[i].first>0){
                  if(-1*neg[j].first < pos[i].first){
                      pos[i].first+=neg[j].first;
                      result+=(-1*neg[j].first*abs(pos[i].second-neg[j].second));
                      neg[j].first=0;
                      k++;
                  }else{
                      neg[j].first=neg[j].first+pos[i].first;
                      result+=(pos[i].first*abs(pos[i].second-neg[j].second));
                      pos[i].first=0;
                  }
              }else{
                  break;
